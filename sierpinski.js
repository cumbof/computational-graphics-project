...
function sierpinski(dim, levels, modelLength) {
	if (dim === undefined || dim === 0)
		...
		return sierpinski2D(dim, levels, modelLength);
	else if (dim === 3 && (modelLength===undefined || modelLength===0))
		return undefined;
	else if (dim === 3 && modelLength>0 && (levels === 0 || levels === undefined)) {
		return simpleSierpinski3D(dim, modelLength);
	}
	else if (dim === 3 && modelLength>0 && levels>0)
		return sierpinski3D(dim, levels, modelLength);

	return undefined;
}

function sierpinski2D(dim, levels, modelLength) {
	if (dim === 2) {
...
function simpleSierpinski3D(dim, modelLength) {
	if (dim === 3) {
...
function sierpinski3D(dim, levels, modelLength) {
	if (dim === 3) {
